...
class uEEPROMLib {
   ...
   private:
    ...
    bool init = false;
#define uEEPROMLIB_STM32_INIT_FIX() \
    {                               \
        if (!init) {                \
            init = true;            \
            _eeprom_read(0);        \
 ...
 */
template <typename TW>
bool uEEPROMLib::eeprom_write(const unsigned int address, const TW data) {
 ...
 */
template <typename TR>
void uEEPROMLib::eeprom_read(const unsigned int address, TR* data) {
