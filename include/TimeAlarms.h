...
class TimeAlarmsClass
{
  ...
  public:
    ...
    AlarmID_t alarmOnce(time_t value, OnTick_t onTickHandler) {
      if (value <= 0 || (unsigned) value > SECS_PER_DAY) return dtINVALID_ALARM_ID;
    ...
    AlarmID_t alarmRepeat(time_t value, OnTick_t onTickHandler) {
      if ((unsigned)value > SECS_PER_DAY) return dtINVALID_ALARM_ID;
    ...
    AlarmID_t alarmRepeat(const timeDayOfWeek_t DOW, const int H, const int M, const int S, OnTickByte_t onTickByteHandler, byte param) {
      time_t value = (DOW - 1) * SECS_PER_DAY + AlarmHMS(H, M, S);
      if (value <= 0) return dtINVALID_ALARM_ID;
