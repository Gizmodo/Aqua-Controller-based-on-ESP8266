  ...
  if (Mode.isEnabled) {
    ...
    if (dtIsAlarm(Mode.alarmType) && nextTrigger <= time) {
      ...
      if (Mode.alarmType == dtExplicitAlarm) {
        ...
        if (value + previousMidnight((unsigned) now()) <= (unsigned) time) {
        ...
        if ((value + previousSunday(now())) <= (unsigned) time) {
...
{
  if ( ! ( (dtIsAlarm(alarmType) && (unsigned) now() < SECS_PER_YEAR) || (dtUseAbsoluteValue(alarmType) && (value == 0)) ) ) {
