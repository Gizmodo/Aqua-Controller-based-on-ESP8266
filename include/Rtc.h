...
class RtcBase {
   public:
    ...
    virtual uint32_t getEpoch() {
        return getSecondsSince2000() + EPOCH_TIME_OFF;
    }  // UNIX time
    virtual void
    get(uint8_t& hour, uint8_t& minute, uint8_t& second, uint16_t& year, uint8_t& month, uint8_t& day, uint8_t& dow) = 0;
    ...
    virtual void setEpoch(uint32_t epoch) {
        setSecondsSince2000(epoch - EPOCH_TIME_OFF);
    }  // UNIX time
